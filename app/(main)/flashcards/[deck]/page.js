...
const use=async({params})=>{
    ...
    return(
        ...
        <div className="flex flex-col min-h-screen bg-[#F5F5FF] pb-10">
            <div className="flex gap-x-5 items-center justify-center">
                <Link href={`/flashcards/${deck}`}>
                    <label className="text-black hover:text-purple-600 cursor-pointer font-light text-sm">Study Mode</label>
                </Link>
                <Link href={`/flashcards/${deck}/quiz`}>
            ...
            <div className="my-3 mx-16">
                <p className="text-4xl font-medium bg-gradient-to-r  from-purple-500  to-fuchsia-500 bg-clip-text text-transparent">{data.title}</p>
            ...
            <div className="grid grid-cols-4 gap-5 mx-16">

                {
                    ...
                    <>
                        {data.message.cards?.map((va)=>{
                            return <Flash xx={va} dec={deck}  key={va._id} />
                        })}
                        <Add params={deck}/>
                    </>
                }
